...
(1|rep/krate) we are nesting rep in krate. you always start with your biggest, which is you block, then your whole plot trt factor (krate), you dont add the nrate because in this case this is the smallest unit and that'll go to the residuals. 
the smallest unit will go to the residuals, so you don't specify it on the random effects (in case we have a p rate)
```{r model}
